...
obstacle_detector_spawnpoints = {
    ...
    "left": carla.Transform(carla.Location(y=-1.5, z=1.3), carla.Rotation(yaw=-90)),
    "right": carla.Transform(carla.Location(y=1.5, z=1.3), carla.Rotation(yaw=90))
}
...
def drawDetectorStatus(image):
    ...
    if detector_notify["front"] != 0 or detector_notify["back"] != 0 or detector_notify["left"] != 0 or detector_notify["right"] != 0:
        ...
        image = cv2.putText(image, f"WARNING", (530, 150), GLOBAL_FONT, 2, custom_color, custom_thickness)
        image = cv2.putText(image, f"obstacle nearby", (540, 180), GLOBAL_FONT, 1, custom_color, custom_thickness)
...
try:
    ...
    camera_spawn_point = carla.Transform(carla.Location(x=-15.0, z=8), carla.Rotation(pitch=-25.0))
